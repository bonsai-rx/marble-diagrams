...
\matrix[row sep=0.5cm]
{
  ...
  \node[complete] (3x0) at (6.6,0) {};
  \\
  
  \node[box,minimum width=7cm] (op) {ElementIndex};
  \\
  
  \draw [time] (0,0) -- (7,0);
  \node[notification] (0x2) at (1,0) {\scriptsize{$0$} \tikz[baseline=-1mm] \node[marble,circle,red] {};};
  \node[notification] (1x2) at (3,0) {\scriptsize{$1$} \tikz[baseline=-1mm] \node[marble,circle,green] {};};
  \node[notification] (2x2) at (5,0) {\scriptsize{$2$} \tikz[baseline=-1mm] \node[marble,circle,blue] {};};
  \node[complete] (3x2) at (6.6,0) {};
  \\
};
